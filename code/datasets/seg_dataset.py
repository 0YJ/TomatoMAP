#!/usr/bin/env python3
"""
Segmentation dataset utilities for TomatoMAP
"""
...
def analyze_dataset_areas(ann_dir, cfg):
    """
    Analyze object area distribution in dataset.
    ...
    for split in ['train', 'val', 'test']:
        ...
        # Warnings
        if small_scaled == 0:
            print(f"\n⚠️  No small objects after scaling - APs metric will be -1")
        if medium_scaled == 0:
            print(f"⚠️  No medium objects after scaling - APm metric will be -1")
        if large_scaled == 0:
            print(f"⚠️  No large objects after scaling - APl metric will be -1")
